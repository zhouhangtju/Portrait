...
class CallRecordEnriched(PortraitBase, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    qa_pairs: Mapped[Optional[str]] = mapped_column(Text,nullable=True,comment="问答对列表（JSON字符串）")
    satisfaction_llm_res: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="满意度模型返回结果")
    emotion_llm_res: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="情绪模型返回结果")
    emotion_asr_res: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="情绪asr模型返回结果")
    complaint_llm_res: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="投诉模型返回结果")
    churn_llm_res: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="流失模型返回结果")
