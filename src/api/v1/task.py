...
class CallRecordItem(BaseModel):
    ...
    unsatisfied_reason: Optional[str] = Field(default=None, description="不满意原因")
    visit_needed: Optional[str] = Field(default=None, description="是否需要上门")
    visit_reason: Optional[str] = Field(default=None, description="上门原因")
...
async def get_task_calls(
    ...
    if visit_needed:
        ...
        if normalized in ("yes", "y", "true", "1", "需要", "要", "需上门", "是"):
            normalized = "yes"
        elif normalized in ("no", "n", "false", "0", "不需要", "无需", "不用", "否"):
            normalized = "no"
        base_conditions.append(CallRecordEnriched.visit_needed == normalized)
    if visit_reason:
        base_conditions.append(CallRecordEnriched.visit_reason == visit_reason)
    ...
    stmt = (
        select(
            ...
            CallRecordEnriched.unsatisfied_reason,
    ...
    items = [
        CallRecordItem(
            ...
            unsatisfied_reason=row.unsatisfied_reason,
