...
class ETLService:
    ...
    async def sync_call_records(
        ...
        print(f"开始同步 {target_date} 的通话记录")
        ...
        #    logger.info(f"测试模式：只处理前20条记录")
        #    source_records = source_records[:5]
    ...
    async def analyze_call_records(
        ...
        logger.info(f"一共有 {len(asr_data)} 条ASR详情记录")
        ...
        for record in records_to_analyze:
            ...
            logger.info(f"callid: {callid}")
            # 获取该通话的 ASR 数据
            call_asr = asr_data.get(callid, {"qa_pairs": [], "task_id": "", "voice_url": []})
            qa_pairs = call_asr.get("qa_pairs", [])
            qa_pairs_str = json.dumps(qa_pairs,ensure_ascii=False,indent=2)
            logger.info(f"task_id: {call_asr.get('task_id')}")
            logger.info(f"qa_pairs: {call_asr.get('qa_pairs')}")
            # 调用规则引擎分析
            result = await rule_engine.analyze_call(
                qa_pairs=call_asr.get("qa_pairs", []),
                task_id=call_asr.get("task_id", ""),
                voice_url=call_asr.get("voice_url", []),
            ...
            updates.append(
                {
                    ...
                    "satisfaction_llm_res": result.satisfaction_llm_res,
                    "emotion": result.emotion,
                    "emotion_llm_res": result.emotion_llm_res,
                    "emotion_asr_res": result.emotion_asr_res,
                    "complaint_risk": result.complaint_risk,
                    "complaint_llm_res":result.complaint_llm_res,
                    "churn_risk": result.churn_risk,
                    "churn_llm_res": result.churn_llm_res,
    ...
    async def extract_voice_urls(self, data) -> List[str]:
        """
    ...
    async def extract_qa_pairs(self, data):
        """
        从通话记录中提取QA对
    ...
    def _fetch_agent_detail_json(
        ...
        url = f"{base_url}/agent-api/user/{user_id}/task/{task_id}/detail/{record_id}"
        logger.info(f"url: {url}, task_id:{task_id}")
        headers = {
    ...
    async def _batch_fetch_asr_details(
        ...
        # 获取涉及的所有分表
        tables = get_tables_for_period(min_date, max_date, "call_record")
        ...
        # 先从 DB 拉取定位 rows
        try:
            base_url, token = await self._get_agent_api_config()
            logger.info(f"base_url: {base_url}")
        except Exception as e:
            logger.error(f"Agent API 配置缺失，无法拉取 ASR 明细: {e}")
            return {}
        ...
        async for session in get_source_db():
            try:
                ...
                for i, row in enumerate(rows):
                    ...
                    qa_pairs = await self.extract_qa_pairs(detail_json)
                    voice_url = await self.extract_voice_urls(detail_json)
                    result_map[callid] = {
                        "qa_pairs": qa_pairs,
                        "task_id": task_id,
                        "voice_url": voice_url
        ...
        async for session in get_portrait_db():
            ...
            for i in range(0, len(updates), batch_size):
                ...
                for idx, update in enumerate(batch):
                    ...
                    params[f"satisfaction_llm_res_{idx}"] = update["satisfaction_llm_res"]
                    params[f"emotion_{idx}"] = update["emotion"]
                    params[f"emotion_llm_res_{idx}"] = update["emotion_llm_res"]
                    params[f"emotion_asr_res_{idx}"] = update["emotion_asr_res"]
                    params[f"complaint_risk_{idx}"] = update["complaint_risk"]
                    params[f"complaint_llm_res_{idx}"] = update["complaint_llm_res"]
                    params[f"churn_risk_{idx}"] = update["churn_risk"]
                    params[f"churn_llm_res_{idx}"] = update["churn_llm_res"]
                ...
                await session.execute(
                    text(f"""
                        ...
                        SET 
                            ...
                            satisfaction_llm_res = v.satisfaction_llm_res,
                            sentiment = v.emotion,
                            emotion_llm_res = v.emotion_llm_res,
                            emotion_asr_res = v.emotion_asr_res,
                            complaint_risk = v.complaint_risk,
                            complaint_llm_res = v.complaint_llm_res,
                            churn_risk = v.churn_risk,
                            churn_llm_res = v.churn_llm_res,
